...
{Snowflake}{San Mateo CA}{}
{
    ...
    \begin{itemize}
        \item Redesigned 20+ compiler stages with a collective blast radius of 17M+ daily queries as part of a migration to a new expression framework.
        \item Engineered large-scale workload validation strategies to roll out migrated compiler stages, ensuring zero regressions across 6B+ daily customer queries.
    ...
    \begin{itemize}
        \item Led design of new SQL syntax enabling multiple aggregation policies on a table, reducing boilerplate and preventing errors in applying privacy constraints.
        \item Designed algorithms to enforce Join Policy semantics on the parse tree of a SQL query, enabling data sharing for over 5000 tables while preserving sensitive information. % the whole equivalence class thing, involved talking to Principals and discussions with product team on whether people would understand it
...
{Intel}{Toronto ON}{}
{\begin{itemize}
    \item Created an FPGA-specific \textbf{LLVM} optimization pass in \textbf{C++} that improved throughput by 15\% across standard \textbf{OpenCL} benchmarks by using scalar evolution analysis to minimize the bitwidth of induction variables.
    \item Debugged complex issues across the hardware-software boundary, from \textbf{SYCL} front-end through \textbf{LLVM} IR transformations to RTL code generation.
\end{itemize}}
...
{\begin{itemize}
    \item Designed an \textbf{MLIR}-based compiler optimization in \textbf{C++} to eliminate extraneous padding in tensor operations, improving throughput by up to 20\% for key \textbf{ONNX} models on custom ML hardware.
    \item Built end-to-end validation pipeline in \textbf{PyTorch} to track compiler QoR metrics for benchmark models, ensuring effectiveness of implemented optimizations.
\end{itemize}}
...
{\begin{itemize}
    \item Designed a compiler in \textbf{Scala} to convert Bayesian network specifications into a \textbf{Verilog} module that performs real-time probabilistic inference.
    \item Defined \textbf{Protobuf}-based specifications for model description and elaboration.
    \item Implemented domain-specific language for expressing Bayesian networks parsed using an \textbf{ANTLR 4} grammar.
