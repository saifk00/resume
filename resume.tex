...
{Snowflake}{San Mateo CA}{}
{
    ...
    \begin{itemize}
        ...
        \item Engineered large-scale workload validation strategies to roll out migrated compiler stages, ensuring minimal regressions across 6B+ daily customer queries.
    ...
    \begin{itemize}
        ...
        \item Designed algorithms to enforce Join Policy semantics on the parse tree of a SQL query, enabling data sharing for over 3000 tables while preserving sensitive information. % the whole equivalence class thing, involved talking to Principals and discussions with product team on whether people would understand it
...
{Intel}{Toronto ON}{}
{\begin{itemize}
    \item Created an FPGA-specific \textbf{LLVM} optimization pass in \textbf{C++} that improved performance by 15\% across standard \textbf{OpenCL} benchmarks by using scalar evolution analysis to narrow induction variables, reducing bandwidth and area requirements on the device.
...
{\begin{itemize}
    \item Optimized memory layout for tensor operations by eliminating extraneous padding, improving throughput by up to 20\% in certain \textbf{ONNX} models on custom ML accelerator hardware.
    \item Built end-to-end validation pipeline in \textbf{PyTorch} to track compiler QoR metrics for important models, ensuring effectiveness of implemented optimizations
\end{itemize}}
